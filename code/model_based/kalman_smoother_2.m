function [Xekf, Xeks] = kalman_smoother_2(Y, X0, P0, Q, R0, Wmean, Vmean, Inits, ...
    VarWinlen1, tau, gamma, VarWinlen2, varargin)
%KALMAN_SMOOTHER_2 The Extended Kalman Filter (EKF) and Extended Kalman Smoother (EKS) for
...
% Original work Copyright (C) 2006  Reza Sameni
% Sharif University of Technology, Tehran, Iran -- LIS-INPG, Grenoble, France
% reza.sameni@gmail.com
%
% Modified by members of the Institute for Electrical Engineering in Medicine, 
...
for k = 1 : Samples,
    ...
    R(2,2) = gamma*R(2,2) + (1-gamma)*mean(mem2(:,2));  % gamma = 1 >> R = R, no adaption
...
Xeks = zeros(size(Xekf));
PSmoothed(:,:,Samples) = Phat(:,:,Samples);
Xeks(:,Samples) = Xekf(:,Samples);
for k = Samples-1 : -1 : 1,
    [A] = Linearization(Xekf(:,k),Wmean,0);
    S = Phat(:,:,k) * A' * inv(Pbar(:,:,k+1));
    Xeks(:,k) = Xekf(:,k) + S * (Xeks(:,k+1) - Xbar(:,k+1));
